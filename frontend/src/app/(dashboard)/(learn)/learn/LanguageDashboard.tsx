...
export default function LanguageDashboard({ languageId }: LanguageDashboardProps) {
    ...
    <div className="container px-4 py-6 md:px-6 md:py-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Track your progress, recent activities, and get recommendations for your {language.name} learning journey.
        </p>
      </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          <Card>
            ...
            <CardContent>
              ...
              <div className="grid grid-cols-3 gap-2 text-xs text-muted-foreground">
                <div>
                  <div className="font-medium">{progress_summary.completed_paths}</div>
                  <div>Completed</div>
                </div>
                <div>
                  <div className="font-medium">{progress_summary.in_progress_paths}</div>
                  <div>In Progress</div>
                </div>
                <div>
                  <div className="font-medium">{progress_summary.not_started_paths}</div>
                  <div>Not Started</div>
                </div>
              </div>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="grid grid-cols-3 gap-2 text-xs text-muted-foreground">
                <div>
                  <div className="font-medium">{progress_summary.lesson_stats.completed}</div>
                  <div>Completed</div>
                </div>
                <div>
                  <div className="font-medium">{progress_summary.lesson_stats.in_progress}</div>
                  <div>In Progress</div>
                </div>
                <div>
                  <div className="font-medium">{progress_summary.lesson_stats.not_started}</div>
                  <div>Not Started</div>
                </div>
              </div>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex items-center justify-between mb-2">
                <span className="text-2xl font-bold">
                  {progress_summary.quiz_stats.completed}/{progress_summary.quiz_stats.total}
                </span>
                <Medal className="w-5 h-5 text-primary" />
              </div>
              <Progress 
                value={progress_summary.quiz_stats.total > 0 
                  ? (progress_summary.quiz_stats.completed / progress_summary.quiz_stats.total) * 100 
                  : 0} 
                className="h-2 mb-2" 
              />
              <div className="text-xs text-muted-foreground">
                ...
                <div>
                  ...
                  {progress_summary.quiz_stats.completed} of {progress_summary.quiz_stats.total}
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex items-center justify-between mb-2">
                <span className="text-2xl font-bold">
                  {vocabulary_stats.learned_vocabulary}/{vocabulary_stats.total_vocabulary}
                </span>
                <Volume2 className="w-5 h-5 text-primary" />
              </div>
              <Progress 
                value={vocabulary_stats.total_vocabulary > 0 
                  ? (vocabulary_stats.learned_vocabulary / vocabulary_stats.total_vocabulary) * 100 
                  : 0} 
                className="h-2 mb-2" 
              />
              <div className="text-xs text-muted-foreground">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="mt-1 text-xs text-muted-foreground">
                          {activity.learning_path && (
                            <span className="mr-2">
                              <span className="font-medium">Path:</span> {activity.learning_path}
                            </span>
                          )}
                          {activity.lesson && (
                            <span className="mr-2">
                              <span className="font-medium">Lesson:</span> {activity.lesson}
                            </span>
                          )}
                          <span>
          ...
          {recommendations.length > 0 ? (
            <div className="space-y-4">
              {recommendations.map((recommendation: any, index: number) => (
                <Card key={`${recommendation.type}-${recommendation.id}-${index}`}>
                  <CardContent className="p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="mt-1 text-xs text-muted-foreground">
                          {recommendation.learning_path && (
                            <div className="mb-1">
                              <span className="font-medium">Path:</span> {recommendation.learning_path}
                            </div>
                          )}
